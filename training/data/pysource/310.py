...
def DenseNet121(classes_number, final_layer_activation):
    ...
    base_model = DenseNet_121(reduction=0.5, weights_path=WEIGHTS_PATH + 'densenet121_weights_tf.h5')
...
def DenseNet169(classes_number):
    ...
    base_model = DenseNet_169(reduction=0.5, weights_path=WEIGHTS_PATH + 'densenet169_weights_tf.h5')
    x = base_model.layers[-3].output
    del base_model.layers[-2:]
    x = Dense(classes_number, activation='sigmoid', name='predictions')(x)
    model = Model(input=base_model.input, output=x)
